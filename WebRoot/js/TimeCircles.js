...
(function($) {
    ...
    var TC_Instance = function(element, options) {
        ...
        if(height === 0 && width > 0) height = width ;//修改宽高
        ...
        this.data.attributes.item_size = Math.min(this.data.attributes.canvas[0].width / 4, this.data.attributes.canvas[0].height); //修改
        this.data.attributes.line_width = this.data.attributes.item_size * this.config.fg_width;//线宽
        this.data.attributes.radius = ((this.data.attributes.item_size * 0.8) - this.data.attributes.line_width) / 2; //内半径
        this.data.attributes.outer_radius = this.data.attributes.radius + 0.5 * Math.max(this.data.attributes.line_width, this.data.attributes.line_width * this.config.bg_width);
//外半径
        // Prepare Time Elements时间元素
        var i = 0;
        for (var key in this.data.text_elements) {
            var textElement = $("<div>");
            textElement.addClass('textDiv_' + key);
            textElement.css("top", Math.round(0.35 * this.data.attributes.item_size));
            textElement.css("left", Math.round(1.5* this.data.attributes.item_size));//文本位置
    ...
    TC_Instance.prototype.updateArc = function() {
		...
		var time_old = {
			second_o : old_diff % 60 ,
			minute_o : (old_diff / 60) % 60
			};
        ...
        var time = {
           /* Days: (diff / 60 / 60 / 24),
            ...
            Minutes: (diff / 60) % 60,
            Seconds: diff % 60*/
			times:time_new.minute_n + ":" + time_new.second_n 
        };
        ...
        var old_time = {
            ...
            Seconds: old_diff % 60*/
			times:time_old.minute_o + ":" + time_old.second_o
        };
        ...
        for (var key in time) {
            // Set the text value
            this.data.text_elements[key].text(/*Math.floor(time[key])*/(time_new.minute_n <10?Math.floor(time_new.minute_n):Math.floor(time_new.minute_n) )+":"+(time_new.second_n < 10?"0"+Math.floor(time_new.second_n):+Math.floor(time_new.second_n) ));
			
            var x = (1.5 * this.data.attributes.item_size) + (this.data.attributes.item_size / 2);
    ...
    TC_Instance.prototype.drawArc = function(x, y, color, pct) {//绘制环形
        ...
        var startAngle = (1 * Math.PI);//起始位置
        var endAngle = (1 * Math.PI) + (2 * pct * Math.PI);//终点位置
